  ...
  <div :id="id" class="section-wrap texture-paper-bkg">
      ...
      <div :class="heroVisibilityClass">
        ...
        <transition name="fade-hero" @after-enter="sectionActive = true">
          <div id="current-sprite-image" v-if="heroActive" :class="'sprite-' + this.id +'-wrap canvas-container'">

              <!-- <canvas id="the-canvas" :class="currentSpriteImage()" width="500" height="500"></canvas> -->
...
export default {
  ...
  methods: {
    ...
    updateHeroClass() {
      var updatedHeroClass = 'fade-me hero-visibility-wrap outer-sprite-container';
    ...
    currentSpriteImage() {
      ...
      // const self = this;
      // function loadCanvas(heroID) {

      var spriteImage = this.id;

      var canvas = document.createElement('canvas'),
        div = document.getElementById('current-sprite-image');
      // div.appendChild(canvas).setAttribute('id', 'the-canvas');
      ...
      function onLoad() {
        ...
        if (loadingCount == 0) {
          ...
          this.animation = TweenLite.to(sprite, 2, {
            frame:       totalFrames - 1,
            repeat:      -1,
            repeatDelay: 0.25,
            yoyo:        true,
      ...
      function draw() {
        ...
        c.clearRect(0, 0, frameSize, frameSize);
        c.drawImage(frame.sheet, frame.x, frame.y, frameSize, frameSize, 0, 0, frameSize, frameSize);

        lastFrame = sprite.frame;
      }

      this.currentSprite = 'sprite-' + `${self.id}` + ' current-animation sprite';
      ...
      // loadCanvas('current-sprite-image');
