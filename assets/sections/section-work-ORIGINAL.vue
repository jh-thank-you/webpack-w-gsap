...
export default {
  ...
  created() {
    ...
    eventBus.$on('getTheWorkExamples', () => {
      // get the Exaples from the array
      this.getExamples();

      if (this.$root.debug) { console.log( this.$route.params.id + ' = this.$route.params.id value when eventBus emits getTheWorkExamples in section-content'); }
  ...
  methods: {
    ...
    updateCornerNav() {
      ...
      if ( this.imagesource == 'modal-print-personal' && this.id == '') {

        if (this.$root.debug) { console.log( this.imagesource + ' = this.id value at the time rater close button was clicked'); }

        if (this.$root.debug) { console.log('Personal, Pharma or Archive Selected'); }

      } else if ( this.imagesource == 'modal-print-pharma') {

        if (this.$root.debug) { console.log( this.imagesource + ' = this.id value at the time rater close button was clicked'); }

        if (this.$root.debug) { console.log('Personal, Pharma or Archive Selected'); }

      } else if ( this.imagesource == 'modal-print-archive') {
        ...
        if (this.$root.debug) { console.log('bringBackCornerNav - route to home page'); }

        // send to new route
        this.$router.push({ path: '/', id: 'section-home' }); // -> /home
      }
    ...
    getExamples() {
      ...
      if (currentID == 'section-work') {
      ...
      } else if (currentID == 'section-personal-work') {
        ...
        currentExamples = [
          ...
          {exampleid: 'modal-print-personal-10', alt: 'Wine Glasses Scratchboard', client: 'Glasses Scratchboard', sector: ['default', 'finearts'], access: 'unlocked' },

          {exampleid: 'modal-print-personal-11', alt: 'Glasses Graphite', client: 'Glasses Graphite', sector: ['default', 'finearts'], access: 'unlocked' },
      ...
      } else if (currentID == 'section-pharma-work') {
      ...
      } else if (currentID == 'section-archived-work') {
